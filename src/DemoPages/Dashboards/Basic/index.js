...
export default class AnalyticsDashboard1 extends Component {
    ...
    componentDidMount(){
        ...
        socket.on("skeleton", data => this.setState({response: data}));
        ...
        const geometryNeck =  new THREE.SphereGeometry(10, 6, 6);  
        ...
        const bonesLimbs = [];
        for (var i=0; i<2; i++){
          bonesLimbs[i] = createBones( -limbHalfHeight, boneLimbHeight, boneLimbCount); // function
        }
        ...
        //     //renderer.setSize(container.offsetWidth*0.5, container.offsetWidth*0.5);   
        //     renderer.setSize(container.offsetWidth*0.5, container.offsetWidth*0.5);           
        // }
        function skinIndexWeigtLatheBody(){
            for ( var i =0; i<geometryBody.vertices.length; i++ ) {   
                const vertexY =  geometryBody.vertices[ i ].y ;                                               // only  y dependet
        ...
        function skinIndexWeightCylinder(geometry){
            for ( var i = 0; i < geometry.vertices.length; i ++ ) {        
                var vertexY =  geometry.vertices[ i ].y  +  limbHalfHeight;             // only  y dependet
                var skinIndex = Math.floor( vertexY  / boneLimbHeight );                // bone 0: bottom  
                var skinWeight = ( vertexY  % boneLimbHeight) /  boneLimbHeight;        // weight
        ...
        function createBones(positionY, height, boneCount){
            var bones = [];                   // base: bone 0
            var basicBone = new THREE.Bone(); // base bone, length 0, not visible
            bones.push( basicBone );          
            basicBone.position.y = positionY; 
            var prevBone = basicBone;             // previous bone for further
            for ( var i = 1; i < boneCount+1 ; i ++ ) {
                var bone = prevBone.clone();                
            ...
            <Fragment>
                <ReactCSSTransitionGroup
                    ...
                    <div>
                        <Row>
                            <Col md="12">
                                <Card className="main-card mb-3 card-border">
                                    <CardBody id="threejs-canvas">
